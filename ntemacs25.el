...
;; Time-stamp: <2018-07-11 14:45 masuda>
;;
...
(set-register ?m '(file . "~/.emacs.d/ntemacs25.el")) ;; emacs25
...
(setq ma:byte-comple-list '("~/.emacs.d/ntemacs25.el"))
  ...
  (let ((n 0))
	(save-excursion
	  ...
	  (while (re-search-forward "^[ 	]+$" nil t)
		(setq n (+ n 1))))
	(if (> n 0)
		(if (y-or-n-p (format "タブ/スペースだけの行 %d 個にマッチしました 変換しますか?" n))
			(progn
			  (save-excursion
				(beginning-of-buffer)
				(replace-regexp "^[ 	]+$" "" nil)))
  ...
  (let ((n 0))
	(save-excursion
	  ...
	  (while (re-search-forward "[ 	]+$" nil t)
		(setq n (1+ n))))
	(if (> n 0)
		(if (y-or-n-p (format "行末のタブ/スペース %d 個にマッチしました 変換しますか?" n))
			(progn
			  (save-excursion
				(beginning-of-buffer)
				(replace-regexp "[ 	]+$" "" nil)))
		...
		(while t
		  (if (> (+ (point) 1) e)
			...
			(if (re-search-forward "[ 	]+" nil t)
			  ...
			  (throw 'loop t)))))
	  (if (> n 0)
		  (if (y-or-n-p (format "タブ/スペースの連続箇所 %d 個にマッチしました スペース▽ひとつに変換しますか?" n))
			  (progn
				(goto-char b)
...
(define-key my-key-map "L" 'flymake-display-err-menu-for-current-line)

(define-key my-key-map "F"
  '(lambda ()
	 "flymake-mode ON/OFF"
	 (interactive)
	 (if flymake-mode
		 (flymake-mode nil) (flymake-mode t))
	 (message "Flymake-mode is %s" (or (and flymake-mode "ON") "OFF") )))
...
;; 2016-10-17 mon 10:41 comment for 25.1
;; ;; 10.10.04(mon)-15:57
;; (require 'zlc)
